...
extract_linear_predictor <- function(inla_model, n, d, Lk_eta) {
    ...
    p_mod <- sapply(p_mod, function(x) x$latent[1:(n * d)])  #L^t eta
    ...
    # a <- proc.time()
    # summary_linear_predictor <- mclapply(p_predictor, function(A) {
    #     t(apply(A, 1, function(x) summary(x)))
    # }, mc.cores = 4)
    # b <-proc.time()
    # b - a
    ...
    marginals_linear_predictor <- p_predictor %>% map(function(cat) {
        apply(cat, 1, function(sim) {
            as.data.frame(density(sim, n = 100)[c("x", "y")])
        })
    })
    b <- proc.time()
    b - a
    ...
    #prueba
    # a2<- proc.time()
    # marginals_linear_predictor2 <- p_predictor %>% map(function(cat) {
    #     cat <- as.data.frame(as.matrix(t(cat)))
    #     sapply(cat, function(sim) {
    #         as.data.frame(density(sim, n = 100)[c("x", "y")])
    #     })
    # })
    # b2 <- proc.time()
    ...
    # a1 <- proc.time()
    # p_alphas <- p_predictor %>% map(exp)
    # marg_alphas <- p_alphas %>% map(function(A) {
    #         summary_alphas <- as.data.frame(t(apply(A, 1, function(x) summary(x))))
    #         marginals_alphas <- apply(A, 1, function(sim) {
    #             dens <- density(sim, n = 75)
    #             data.frame(x = dens$x, y = dens$y)
    #         })
    #
    #         list(summary_alphas = summary_alphas,
