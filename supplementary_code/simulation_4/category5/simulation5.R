...
simulations_just_intercepts <- function(n,   x =c(2.4, 1.2, 3.1, 1.3,2))
{
  ### --- 2. Simulation data --- ####
  set.seed(1000)
  ...
  if(file.exists(paste0("model_jags_", n,".RDS"))){
    ...
    parameters <- c('beta0', 'alpha[1,1]', 'alpha[1,2]', 'alpha[1,3]', 'alpha[1,4]', 'alpha[1,5]', 'mu')
    ...
    log(alpha[i,5]) <- beta0[5]

    }

    alpha0 <- alpha[1,1] + alpha[1,2] + alpha[1,3] + alpha[1,4] + alpha[1,5]
    mu[1] <- alpha[1,1]/alpha0
    mu[2] <- alpha[1,2]/alpha0
    mu[3] <- alpha[1,3]/alpha0
    mu[4] <- alpha[1,4]/alpha0
    mu[5] <- alpha[1,5]/alpha0
  ...
  if(file.exists(paste0("model_inla_", n,".RDS"))){
    ...
    t <- proc.time() # Measure the time
    model.inla <- dirinlareg( formula  = y ~ 1 | 1 | 1 | 1 | 1  ,
                              y        = y,
                              data.cov = V,
                              prec     = 0.0001,
                              verbose  = TRUE,
  ...
  rownames(result_mus) <- paste0("mu", 1:d)
  ...
  for (i in 1:length(model.inla$marginals_fixed))
  {
    ...
    p2[[i]] <- p2[[i]] + geom_vline(xintercept = x[seq(1,d, by=1)][i])
  ...
  gridExtra::grid.arrange(p2[[1]], p2[[2]], p2[[3]], p2[[4]], p2[[5]],
               p3[[1]], p3[[2]], p3[[3]], p3[[4]], p3[[5]], ncol = 5)
  dev.off()
...
#                         mc.cores = 3)
setwd("5Positive")
a <- lapply(n, simulations_just_intercepts, x =c(2.4, 1.2, 3.1, 1.3, 2))
